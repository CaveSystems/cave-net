    ...
    internal static ImapMailboxInfo FromAnswer(string name, ImapAnswer answer)
    {
        ...
        foreach (var line in answer.GetDataLines())
        {
            if (line.StartsWith("* "))
            {
                ...
                {
                    case "FLAGS":
                        result.Flags = parts[2].UnboxBrackets().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                        continue;
                    case "OK":
                        if (parts.Length > 2)
                        {
                            ...
                            {
                                case "PERMANENTFLAGS":
                                    result.PermanentFlags = subParts[1].UnboxBrackets().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                                    continue;
                                case "UIDVALIDITY":
