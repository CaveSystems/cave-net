...
namespace Cave.Net
{
    ...
    public class TcpAsyncStream : Stream
    {
        ...
        /// Default is false buffering all writes. You need to set this to true if you use the clients <see cref="TcpAsyncClient.Send(byte[])"/> function and
        /// stream writing at the same time.
        /// </summary>
        ...
        /// <summary>
        /// Reads data from the the buffers. A maximum of count bytes is read but if less is available any number of bytes may be read. If no bytes are
        /// available the read method will block until at least one byte is available, the connection is closed or the timeout is reached.
        /// </summary>
